...
export class SuperSimpleQueueHelper implements ISuperSimpleQueueHelper {
					...
					this.notificationsService.handleNotifications(statusChangeResult.monitor, status, decision).catch((error: unknown) => {
						this.logger.error({
							...
							stack: error instanceof Error ? error.stack : undefined,
								...
								if (
									monitor.status === "down" &&
									Array.isArray(monitor.escalationPolicies) &&
									monitor.escalationPolicies.length > 0
								) {
	...
	private evaluateMonitorAction(statusChangeResult: StatusChangeResult): MonitorActionDecision {
		 ...
		 */
		async handleEscalationStepJob({ monitorId, teamId, incidentId: _incidentId, stepIndex }: { monitorId: string; teamId: string; incidentId?: string; stepIndex: number }) {
			...
			if (monitor.status !== "down") {
				this.logger.info({ message: `Monitor ${monitorId} is not down, skipping escalation`, service: SERVICE_NAME, method: "handleEscalationStepJob" });
				return;
			}

			const step = monitor.escalationPolicies?.[stepIndex];
			if (!step) {
				this.logger.warn({ message: `No escalation step ${stepIndex} for monitor ${monitorId}`, service: SERVICE_NAME, method: "handleEscalationStepJob" });
				return;
			}
